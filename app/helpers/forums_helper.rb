module ForumsHelper
  ...
  def fetch_metric_topics forum, user_or_enterprise
    if user_or_enterprise && user_or_enterprise.id != 0
      ...
      elsif user_or_enterprise.instance_of? Forum
        # view for a particular forum
        @owned_open_topics = user_or_enterprise.topics.tracked.open.sort_by{|t| t.days_open * -1}
        @owned_closed_topics = user_or_enterprise.topics.tracked.closed.closed_after(@weeks[8])
      else
        # it's based on user
