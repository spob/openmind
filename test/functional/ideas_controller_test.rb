...
class IdeasControllerTest < Test::Unit::TestCase
  fixtures :ideas, :users, :roles_users
  ...
  def test_show
    ...
    user_idea_read = UserIdeaRead.find_by_idea_id(@first_id)
    time = user_idea_read.last_read
    
    sleep 1 # sleep to make sure times are different
    get :show, :id => @first_id
    assert UserIdeaRead.find_by_idea_id(@first_id).last_read.to_f > time.to_f
  end
